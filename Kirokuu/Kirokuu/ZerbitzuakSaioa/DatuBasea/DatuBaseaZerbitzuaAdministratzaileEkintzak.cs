...
public sealed partial class DatuBaseaZerbitzua
{
    ...
    private async Task BermatuTursoSektoreaTextMigrazioaAsync(ITursoSqlEgikaritzailea bezeroa, CancellationToken cancellationToken)
    {
        const string sql = """
            UPDATE Erabiltzaileak
            SET Sektorea = CASE Sektorea
                WHEN '1' THEN 'Finantzak'
                WHEN '2' THEN 'Marketina'
                WHEN '3' THEN 'Salmentak'
                ELSE ''
            END
            WHERE Sektorea IN ('1','2','3');
            """;
        await bezeroa.ExekutatuAsync(sql, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_urrunTursoModua)
            return await ZerrendatuTxostenGuztiekTursoAsync(sektoreIzena, egoeraIragazkia, cancellationToken).ConfigureAwait(false);
        ...
        const string sqlOinarria = """
            SELECT
              ...
              COALESCE((SELECT TRIM(COALESCE(a.Izena,'') || ' ' || COALESCE(a.Abizena,''))
                        FROM Erabiltzaileak a
                        WHERE b.AdminDNI IS NOT NULL AND b.AdminDNI <> '' AND a.DNI = b.AdminDNI
                        LIMIT 1), '') AS AdminTestua
            FROM BidaiaTxostenak b
            INNER JOIN Erabiltzaileak e ON e.ErabiltzaileId = b.ErabiltzaileId
            """;

        var sektoreaIragazi = !string.IsNullOrWhiteSpace(sektoreIzena);
        var egoeraIragazi = !string.IsNullOrWhiteSpace(egoeraIragazkia);

        var baldintzak = new List<string>();
        var parametroak = new List<object>();
        if (sektoreaIragazi) { baldintzak.Add("e.Sektorea = ?"); parametroak.Add(sektoreIzena!.Trim()); }
        if (egoeraIragazi) { baldintzak.Add("b.Egoera = ?"); parametroak.Add(egoeraIragazkia!.Trim()); }

        var whereZatia = baldintzak.Count > 0 ? " WHERE " + string.Join(" AND ", baldintzak) : string.Empty;
        var sql = sqlOinarria + whereZatia + " ORDER BY b.SorkuntzaData DESC";
        return await _sqliteKonexioa!.QueryAsync<TxostenOnarpenLaburpena>(sql, parametroak.ToArray()).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken)
    {
        return await ExekutatuTursoanAsync(async bezeroa =>
        {
            ...
            var baldintzak = new List<string>();
            var loturak = new List<object?>();
            if (sektoreaIragazi) { baldintzak.Add("e.Sektorea = ?"); loturak.Add(LibsqlLoturaNormalizatua(sektoreIzena!.Trim())); }
            if (egoeraIragazi) { baldintzak.Add("b.Egoera = ?"); loturak.Add(LibsqlLoturaNormalizatua(egoeraIragazkia!.Trim())); }

            var whereZatia = baldintzak.Count > 0 ? " WHERE " + string.Join(" AND ", baldintzak) : string.Empty;
            var sql = sqlOinarria + whereZatia + " ORDER BY b.SorkuntzaData DESC;";

            var emaitza = await bezeroa.ExekutatuAsync(sql, cancellationToken, loturak.ToArray()).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_urrunTursoModua)
            return await ZerrendatuTxostenOnarpenLaburrakTursoAsync(egoeraIragazkia, sektoreIzena, cancellationToken).ConfigureAwait(false);

        var iragazkiEgoera = !string.IsNullOrWhiteSpace(egoeraIragazkia);
        var iragazkiSektorea = !string.IsNullOrWhiteSpace(sektoreIzena);
        ...
        if (iragazkiEgoera && iragazkiSektorea)
        {
            sql = """
                  ...
                  ORDER BY b.SorkuntzaData DESC
                  """;
            return await _sqliteKonexioa!.QueryAsync<TxostenOnarpenLaburpena>(sql, egoeraIragazkia!.Trim(), sektoreIzena!.Trim()).ConfigureAwait(false);
        }
        ...
        if (iragazkiSektorea)
        {
            sql = """
                  ...
                  ORDER BY b.SorkuntzaData DESC
                  """;
            return await _sqliteKonexioa!.QueryAsync<TxostenOnarpenLaburpena>(sql, sektoreIzena!.Trim()).ConfigureAwait(false);
        }
        ...
        return await ExekutatuTursoanAsync(async bezeroa =>
        {
            ...
            else if (iragazkiEgoera && iragazkiSektorea)
            {
                ...
                emaitza = await bezeroa.ExekutatuAsync(sql, cancellationToken,
                    ...
                    LibsqlLoturaNormalizatua(sektoreIzena!.Trim())).ConfigureAwait(false);
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        txostena.AdminOharra = string.Equals(egoeraBerria, TxostenEgoera.Ukatua, StringComparison.Ordinal)
            ...
            : null;
        txostena.AzkenEguneraketa = orain;
    ...
    private async Task AdministratzaileProbakoDatuakTursoAsync(ITursoSqlEgikaritzailea bezeroa, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var langEmaitza = await bezeroa.ExekutatuAsync(
                "SELECT ErabiltzaileId, DNI FROM Erabiltzaileak WHERE Rola = ? AND DNI <> '' LIMIT 1;",
                cancellationToken,
            ...
            if (langileLerro is null || langileLerro.Count < 2)
                return;
