...
public sealed partial class DatuBaseaZerbitzua
{
    ...
    public async Task<Erabiltzailea> TxertatuErabiltzaileaAsync(Erabiltzailea erabiltzailea, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(sqlEx, "Erabiltzailea txertatzean: datu bikoiztua (constraint). Modua: {Modua}.", _urrunTursoModua ? "Turso" : "SQLite");
            GarapenLogaWarning($"Erabiltzailea txertatzean: datu bikoiztua. Modua: {(_urrunTursoModua ? "Turso" : "SQLite")}.");
            throw;
        }
        catch (SQLiteException sqlEx)
        {
            _logger.LogError(sqlEx, "Erabiltzailea txertatzean SQLite errorea. Modua: {Modua}.", _urrunTursoModua ? "Turso" : "SQLite");
            GarapenLogaError(sqlEx, $"Erabiltzailea txertatzean SQLite errorea. Modua: {(_urrunTursoModua ? "Turso" : "SQLite")}.");
            throw;
        }
        catch (HttpRequestException httpEx)
        {
            _logger.LogError(httpEx, "Erabiltzailea txertatzean Turso/sare errorea.");
            GarapenLogaError(httpEx, "Erabiltzailea txertatzean Turso/sare errorea.");
            throw;
        }
        catch (TaskCanceledException tcEx)
        {
            _logger.LogWarning(tcEx, "Erabiltzailea txertatzean denbora muga gainditu da.");
            GarapenLogaWarning("Erabiltzailea txertatzean denbora muga gainditu da.");
            throw;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Erabiltzailea txertatzean ustekabeko errorea. Modua: {Modua}.", _urrunTursoModua ? "Turso" : "SQLite");
            GarapenLogaError(ex, $"Erabiltzailea txertatzean ustekabeko errorea. Modua: {(_urrunTursoModua ? "Turso" : "SQLite")}.");
            throw;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(sektoreIragazkia))
        {
            return await _sqliteKonexioa!.QueryAsync<ErabiltzaileLaburpena>(
                ...
                sektoreIragazkia.Trim()).ConfigureAwait(false);
        }
    ...
    private async Task HasieratuBarneanAsync()
    {
        ...
        {
            _logger.LogError(sqlEx, "Datu-basea hasieratzean SQLite errorea.");
            GarapenLogaError(sqlEx, "Datu-basea hasieratzean SQLite errorea.");
            throw;
        }
        catch (HttpRequestException httpEx)
        {
            _logger.LogError(httpEx, "Datu-basea hasieratzean Turso/sare errorea.");
            GarapenLogaError(httpEx, "Datu-basea hasieratzean Turso/sare errorea.");
            throw;
        }
        catch (TaskCanceledException tcEx)
        {
            _logger.LogWarning(tcEx, "Datu-basea hasieratzean denbora muga gainditu da.");
            GarapenLogaWarning("Datu-basea hasieratzean denbora muga gainditu da.");
            throw;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Datu-basea hasieratzean ustekabeko errorea.");
            GarapenLogaError(ex, "Datu-basea hasieratzean ustekabeko errorea.");
            throw;
        }
    }
    ...
    private async Task BermatuSqliteErabiltzaileEskemaAsync()
    {
        ...
        var sektoreaZutabeenMotak = await IrakurriSqliteErabiltzaileZutabeenMotakAsync().ConfigureAwait(false);
        if (!sektoreaZutabeenMotak.ContainsKey("Sektorea"))
        {
            await _sqliteKonexioa!.ExecuteAsync("ALTER TABLE Erabiltzaileak ADD COLUMN Sektorea TEXT NOT NULL DEFAULT ''").ConfigureAwait(false);
        ...
        else if (sektoreaZutabeenMotak.TryGetValue("Sektorea", out var sektoreaMotaUnea)
                 && sektoreaMotaUnea.StartsWith("INT", StringComparison.OrdinalIgnoreCase))
        {
            await BerregokituSqliteSektoreaTextAsync().ConfigureAwait(false);
        }
        zutabeak = await IrakurriSqliteErabiltzaileZutabeakAsync().ConfigureAwait(false);
    ...
    // Zutabe bakoitzaren izena -> SQLite-ren type ("INTEGER", "TEXT", ...). Sektorea INTEGER->TEXT migrazioa erabakitzeko.
    private async Task<Dictionary<string, string>> IrakurriSqliteErabiltzaileZutabeenMotakAsync()
    {
        var zutabeak = await _sqliteKonexioa!.QueryAsync<SqliteZutabea>("PRAGMA table_info('Erabiltzaileak')").ConfigureAwait(false);
        var hiztegia = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        foreach (var z in zutabeak)
        {
            if (string.IsNullOrWhiteSpace(z.Izena))
                continue;
            hiztegia[z.Izena] = z.Mota ?? string.Empty;
        }
        return hiztegia;
    }

    // Sektorea INTEGER -> TEXT: taula berreraiki Erabiltzaileak_migr via CASE 1='Finantzak' 2='Marketina' 3='Salmentak'.
    private async Task BerregokituSqliteSektoreaTextAsync()
    {
        _logger.LogWarning("SQLite Erabiltzaileak.Sektorea INTEGER -> TEXT migrazioa hasi da.");
        GarapenLogaWarning("SQLite Erabiltzaileak.Sektorea INTEGER -> TEXT migrazioa hasi da.");

        // Egungo zutabe-zerrenda jaso (SELECT eraikitzeko, opzionalak agertzen ez direnean ez huts egiteko).
        var zutabeenMotak = await IrakurriSqliteErabiltzaileZutabeenMotakAsync().ConfigureAwait(false);
        var zutabeIzenakSekuentzian = new[]
        {
            "ErabiltzaileId", "Izena", "Abizena", "Abizena2", "DNI", "Email",
            "Kargoa", "Sektorea", "KargoarenIdentifikatzailea", "Rola",
            "SorkuntzaData", "Pasahitza", "Aktiboa", "SaioHasieraSaiakerak",
            "SaioaBlokeoaAmaieraUtc"
        };
        var zutabeakBetetzen = zutabeIzenakSekuentzian.Where(z => zutabeenMotak.ContainsKey(z)).ToArray();
        var selectZatiak = zutabeakBetetzen.Select(z => z.Equals("Sektorea", StringComparison.OrdinalIgnoreCase)
            ? "CASE Sektorea WHEN 1 THEN 'Finantzak' WHEN 2 THEN 'Marketina' WHEN 3 THEN 'Salmentak' ELSE '' END AS Sektorea"
            : z).ToArray();
        var insertZutabeak = string.Join(", ", zutabeakBetetzen);
        var selectKatea = string.Join(", ", selectZatiak);

        await _sqliteKonexioa!.RunInTransactionAsync(konexioa =>
        {
            konexioa.Execute("DROP TABLE IF EXISTS Erabiltzaileak_migr;");
            konexioa.Execute("""
                CREATE TABLE Erabiltzaileak_migr (
                    ErabiltzaileId INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                    Izena TEXT NOT NULL,
                    Abizena TEXT NOT NULL,
                    Abizena2 TEXT NOT NULL DEFAULT '',
                ...
                """);
            konexioa.Execute($"INSERT INTO Erabiltzaileak_migr ({insertZutabeak}) SELECT {selectKatea} FROM Erabiltzaileak;");
            konexioa.Execute("DROP TABLE Erabiltzaileak;");
            konexioa.Execute("ALTER TABLE Erabiltzaileak_migr RENAME TO Erabiltzaileak;");
        }).ConfigureAwait(false);

        _logger.LogWarning("SQLite Erabiltzaileak.Sektorea INTEGER -> TEXT migrazioa burutu da.");
        GarapenLogaWarning("SQLite Erabiltzaileak.Sektorea INTEGER -> TEXT migrazioa burutu da.");
    }
    ...
    private async Task<Erabiltzailea> TxertatuTursoAsync(Erabiltzailea erabiltzailea, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                const string sql = """
                    INSERT INTO Erabiltzaileak (Izena, Abizena, Abizena2, DNI, Email, Kargoa, Sektorea, KargoarenIdentifikatzailea, Rola, SorkuntzaData, Pasahitza, Aktiboa, SaioHasieraSaiakerak, SaioaBlokeoaAmaieraUtc)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
                    """;
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        const string sql = "SELECT ErabiltzaileId, Izena, Abizena, Abizena2, DNI, Email, Kargoa, Sektorea, KargoarenIdentifikatzailea, Rola, SorkuntzaData, Pasahitza, Aktiboa, COALESCE(SaioHasieraSaiakerak, 0) AS SaioHasieraSaiakerak, SaioaBlokeoaAmaieraUtc FROM Erabiltzaileak WHERE Email = ? LIMIT 1;";
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        const string sql = "SELECT ErabiltzaileId, Izena, Abizena, Abizena2, DNI, Email, Kargoa, Sektorea, KargoarenIdentifikatzailea, Rola, SorkuntzaData, Pasahitza, Aktiboa, COALESCE(SaioHasieraSaiakerak, 0) AS SaioHasieraSaiakerak, SaioaBlokeoaAmaieraUtc FROM Erabiltzaileak WHERE ErabiltzaileId = ? LIMIT 1;";
    ...
    private async Task EguneratuErabiltzaileaTursoAsync(Erabiltzailea erabiltzailea, CancellationToken cancellationToken)
    {
        await ExekutatuTursoanAsync(async bezeroa =>
        {
            const string sql = """
                ...
                SET Izena = ?, Abizena = ?, Abizena2 = ?, DNI = ?, Email = ?, Kargoa = ?, Sektorea = ?, KargoarenIdentifikatzailea = ?, Rola = ?, SorkuntzaData = ?, Pasahitza = ?, Aktiboa = ?, SaioHasieraSaiakerak = ?, SaioaBlokeoaAmaieraUtc = ?
        ...
        return await ExekutatuTursoanAsync(async bezeroa =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(sektoreIragazkia))
            {
                const string sqlIragazia = """
                    ...
                    ORDER BY Izena COLLATE NOCASE, Abizena COLLATE NOCASE;
                    """;
                var emaitzaIragazia = await bezeroa.ExekutatuAsync(
                    sqlIragazia,
                    cancellationToken,
                    (int)ErabiltzaileRola.Langilea,
                    sektoreIragazkia.Trim()).ConfigureAwait(false);
                return MapeatuLangileLaburpenak(emaitzaIragazia);
            }
    ...
    private static (string Sektorea, int KargoarenIdentifikatzailea, string KargoarenTestua) EskuratuGarapenAdminSektoreaEtaKargoarenBalioak(string kargoarenTestuZaharra)
    {
        var sektoreLehenetsia = AdministratzaileOrganizazioLehenetsia.Sektorea;
        var kargoLehenetsia = AdministratzaileOrganizazioLehenetsia.KargoarenIdentifikatzailea;
        // KIROKU_GARAPEN_ADMIN_SEKTOREA: orain sektore izena (kasu honetan "Finantzak"/"Marketina"/"Salmentak").
        ...
        var sektor = string.IsNullOrWhiteSpace(sektorEnv) ? sektoreLehenetsia : sektorEnv;
